...
```{r} 
unrecovered=df$confirmed - (df$death + df$recovered)
valueBox( 
value = paste(format(sum(unrecovered, na.rm = TRUE), big.mark = ","), " (", 
round(100 * sum(unrecovered, na.rm = TRUE) / sum(df$confirmed), 1),
"%)",
sep = "" 
),
caption = "Active cases (% of )", icon = "fas fa-ambulance", 
color = active_color 
) 
...
### **Daily cumulative cases by type** (only)
...
```{r}
all_confirmed_coun <- coronavirus %>%
...
color<-rep('rgba(204,204,204,1)', nrow(all_confirmed_coun))
color[all_confirmed_coun[['country']]==Country]<-'rgba(222,45,38,0.8)'
fig <- all_confirmed_coun %>% plotly::plot_ly(x =~reorder(country,total), y=~total,
             type = "bar", marker = list(color = color)) 
fig<-plotly::layout(fig,yaxis=list(type='log', title='case number'), xaxis=list(title='Country'))
...
#daily_confirmed$Country<-daily_confirmed[,Country]
#names(daily_confirmed)[names(daily_confirmed)==Country]<-"Country"


#----------------------------------------
# Plotting the data
p<-plotly::plot_ly()
for(Coun in c(Country, ListeOtherCount)){
p<-p %>%  plotly::add_trace(
    x=daily_confirmed[['date']] ,
    y=daily_confirmed[[Coun]],
    type = "scatter",
    mode = "lines+markers",
    name = Coun
  )
...
```{r daily_summary}
df_EU <- coronavirus %>%
  dplyr::filter(Country.Region %in% c(ListeOtherCount,Country)) %>%
  dplyr::group_by(Country.Region, type) %>%
  dplyr::summarise(total = sum(cases)) %>%
  tidyr::pivot_wider(
    names_from = type,
...
cv_data_for_plot <- coronavirus %>%
  # dplyr::filter(Country.Region == "Zimbabwe") %>%
...
The code behind this dashboard is adapation of on [GitHub](https://github.com/AntoineSoetewey/coronavirus_dashboard){target="_blank"}.
